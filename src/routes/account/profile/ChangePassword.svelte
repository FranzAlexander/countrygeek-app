...
>
	<div class="mb-4 flex w-full items-center justify-between border-b-2 border-gray-400 p-2">
		<button
			type="button"
			class="inline-flex items-center rounded-lg bg-primary px-5 py-2.5 text-center text-sm font-medium text-secondary hover:bg-accent focus:outline-none focus:ring-4 focus:ring-primary-20 {disabled
				? ''
				: 'hidden'}"
			on:click={toggleEdit}>Change Password</button
		>
	</div>
