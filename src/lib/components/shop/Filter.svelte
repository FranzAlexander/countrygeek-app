...
<div
	class="absolute top-10 z-10 max-h-60 w-32 max-w-sm overflow-auto rounded-lg border border-gray-400 bg-gray-300 py-1 {showFilters ===
	false
		? 'hidden'
		: ''}"
>
	{#each filters as specFilter}
		<ul class="text-sm text-gray-700 dark:text-gray-200">
			<li class="flex flex-col first:rounded-t-lg last:rounded-none">
				<span class="bg-gray-300 px-4 py-1 font-body text-gray-900">{specFilter.specName}</span>
				<ul class="">
					{#each specFilter.specOption as specOption}
						<li>
							<button
								class="flex w-full justify-between bg-white px-4 py-1 text-left font-sans text-sm hover:bg-gray-200"
